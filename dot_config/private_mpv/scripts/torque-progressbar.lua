	...
	local _base_0 = {
		update = function(self, now)
			if self.isReversed then
				self.linearProgress = math.max(
					0,
					math.min(1, self.linearProgress + (self.lastUpdate - now) * self.durationR)
				)
				...
				self.linearProgress = math.max(
					0,
					math.min(1, self.linearProgress + (now - self.lastUpdate) * self.durationR)
				)
	...
	local _base_0 = {
		reconfigure = function(self)
			...
			self.line[2] = ("%g,%g"):format(
				math.min(Window.w - rightMargin, math.max(leftMargin, Mouse.x)),
				self.position
			)
